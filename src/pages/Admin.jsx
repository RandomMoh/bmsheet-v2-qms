...
async function exportUserPDF(u) {
  ...
  const bgHeader = [241, 245, 249]
  const borderSubtle = [226, 232, 240]
  
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(22)
  doc.setTextColor(...primaryText)
  doc.text(u.name, 14, 22)
  ...
  autoTable(doc, {
    ...
    headStyles: { textColor: secondaryText, fontSize: 8, fontStyle: 'bold', cellPadding: { top: 4, bottom: 4 } },
    bodyStyles: { textColor: primaryText, fontSize: 16, fontStyle: 'bold' },
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: bgHeader, textColor: primaryText, fontSize: 8, fontStyle: 'bold', halign: 'center', lineWidth: 0 },
    bodyStyles: { fontSize: 10, halign: 'center', textColor: primaryText, lineWidth: 0 },
    columnStyles: { 0: { halign: 'left', fontStyle: 'bold', textColor: secondaryText, fontSize: 8 } },
    styles: { cellPadding: 5, lineColor: borderSubtle, lineWidth: 0.1 }
  })
  ...
  if (typeEntries.length) {
    autoTable(doc, {
      ...
      headStyles: { fillColor: bgHeader, textColor: primaryText, fontSize: 8, fontStyle: 'bold', lineWidth: 0 },
      bodyStyles: { fontSize: 9, textColor: primaryText, lineWidth: 0 },
      styles: { cellPadding: 4, lineColor: borderSubtle, lineWidth: 0.1 }
    })
  }
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: bgHeader, textColor: primaryText, fontSize: 7, fontStyle: 'bold', lineWidth: 0 },
    bodyStyles: { fontSize: 7, textColor: primaryText, lineWidth: 0 },
    alternateRowStyles: { fillColor: [250, 250, 250] },
    styles: { cellPadding: 3, lineColor: borderSubtle, lineWidth: 0.1, overflow: 'ellipsize' },
    columnStyles: { 
      0: { cellWidth: 10 }, 
      1: { cellWidth: 55 },
      2: { cellWidth: 35 },
  ...
  doc.setFontSize(8); doc.setTextColor(...secondaryText)
  doc.text('QMS \u00a9 Benchmark Studio \u2014 Confidential', 14, doc.internal.pageSize.height - 10)
...
export default function AdminPortal() {
  ...
  const exportOrdersPDF = async () => {
    ...
    doc.text('QMS \u2014 All Orders Export', 14, 22)
    doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(120, 120, 120)
    doc.text(`Exported: ${new Date().toLocaleString()}   //   Total Records: ${filtered.length}`, 14, 30)
    
    autoTable(doc, {
      startY: 40, theme: 'plain',
      head: [GRID_COLS.map(c => c.label)],
      body: filtered.map(o => GRID_COLS.map(c => String(o[c.key] ?? '\u2014'))),
      headStyles: { textColor: [20, 20, 20], fontSize: 8, fontStyle: 'bold', cellPadding: { top: 6, bottom: 6 } },
      bodyStyles: { fontSize: 8, textColor: [80, 80, 80] },
      alternateRowStyles: { fillColor: [251, 252, 253] },
      styles: { cellPadding: 4, overflow: 'ellipsize', lineColor: [235, 235, 235], lineWidth: { bottom: 0.1 } },
    })
  ...
  const exportAllPDF = async () => {
    ...
    const doc = new jsPDF('l', 'mm', 'a3')
    doc.setFont('helvetica', 'bold'); doc.setFontSize(22); doc.setTextColor(20, 20, 20)
    doc.text('QMS Master Orders Export', 14, 22)
    doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(120, 120, 120)
    doc.text(`Generated: ${new Date().toLocaleString()}   //   Total Queries: ${filtered.length}`, 14, 30)
